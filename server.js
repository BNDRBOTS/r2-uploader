...
function authMiddleware(req, res, next) {
  ...
  if (token && secureCompare(token, validToken)) return next();

  // file serving is public
  if ((req.method === 'GET' || req.method === 'HEAD' || req.method === 'OPTIONS') && req.path.startsWith('/files/')) {
    return next();
  }
...
function serveIndexWithCSP(_req, res) {
  ...
  let html;
  try {
    const resolvedPath = fs.existsSync(htmlPath) ? htmlPath : path.join(__dirname, 'index.html');
    html = fs.readFileSync(resolvedPath, 'utf8');
  } catch (err) {
...
app.post('/upload', uploadLimiter, (req, res) => {
  ...
  busboy.on('file', (_fieldname, fileStream, info) => {
    ...
    if (!mimeType || !ALLOWED_MIMES.test(mimeType)) mimeType = guessMimeType(filename);
    
    if (!ALLOWED_MIMES.test(mimeType)) {
      fileStream.resume();
      results.push({ originalName: filename, success: false, error: `Unsupported type: ${mimeType}` });
      return;
    }
    ...
    const uploadPromise = (async () => {
      try {
        const upload = new Upload({
          client: s3,
          params: { Bucket: R2_BUCKET, Key: safeName, Body: fileStream, ContentType: mimeType },
          leavePartsOnError: false,
        });
        await upload.done();
        results.push({ originalName: filename, success: true, url: getPublicUrl(req, safeName), filename: safeName });
      } catch (err) {
        console.error(`Upload failed for ${filename}:`, err);
        results.push({ originalName: filename, success: false, error: 'Storage upload failed.' });
      } finally {
  ...
  busboy.on('error', (err) => {
    if (aborted) return;
    aborted = true;
    for (const stream of activeStreams) { if (!stream.destroyed) stream.destroy(); }
    if (err.code === 'LIMIT_FILE_SIZE') return res.status(400).json({ success: false, error: 'File exceeds 5GB limit.' });
    if (err.code === 'LIMIT_FILES') return res.status(400).json({ success: false, error: 'Max 20 files per cycle.' });
    return res.status(500).json({ success: false, error: 'Upload sequence aborted.' });
  });
...
// ── Global Preflight for Files (Guarantees Embeddability) ──
app.options('/files/:key', (req, res) => {
...
// ── File serving with conditional requests & permissive CORS ──
async function serveFile(req, res, method) {
  ...
  try {
    ...
    let contentType = headResp.ContentType;
    ...
    // Unrestricted Embedding Logic Applied Here
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Cross-Origin-Resource-Policy', 'cross-origin');
    res.set('Access-Control-Allow-Methods', 'GET, HEAD, OPTIONS');
    res.set('Timing-Allow-Origin', '*');
    res.removeHeader('X-Frame-Options'); // Allows external iframing (PDFs/Videos)
    res.removeHeader('Content-Security-Policy'); // Prevents host CSP blocking media streams
...
app.patch('/files/:key', actionLimiter, express.json(), async (req, res) => {
  ...
  let newKey = newExt ? sanitizeFilename(newName) : sanitizeFilename(newName) + oldExt;

  if (newKey.length > 1024) return res.status(400).json({ success: false, error: 'New name is too long.' });
  if (newKey === oldKey) return res.json({ success: true, newKey, message: 'Name unchanged.' });
  ...
  try {
    ...
    res.json({ success: true, newKey, url: getPublicUrl(req, newKey) });
  } catch (err) {
